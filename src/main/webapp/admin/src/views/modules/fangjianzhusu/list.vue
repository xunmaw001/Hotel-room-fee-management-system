        ...
        <div v-if="showFlag">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('fangjianzhusu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/lvshekefangshoufei/upload/fangjianzhusuMuBan.xls"
                        >批量导入住宿信息数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('fangjianzhusu','导入导出')"
                                ...
                                :on-success="fangjianzhusuUploadSuccess"
                                :on-error="fangjianzhusuUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('fangjianzhusu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "fangjianzhusu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="fangjianzhusuName"
                                   header-align="center"
                                   label="预约用户名称">
                        <template slot-scope="scope">
                            {{scope.row.fangjianzhusuName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="fangjianzhusuPhone"
                                   header-align="center"
                                   label="联系方式">
                        <template slot-scope="scope">
                            {{scope.row.fangjianzhusuPhone}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="fangjianzhusuTime"
                                   header-align="center"
                                   label="预约时间">
                        <template slot-scope="scope">
                            {{scope.row.fangjianzhusuTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="zhusuNumber"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="总金额">
                        <template slot-scope="scope">
                            {{(scope.row.zhusuNumber * scope.row.fangjianMoney).toFixed(2)}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                                     ...
                                     label="操作">
                        <template slot-scope="scope">
                            <el-button v-if="isAuth('fangjianzhusu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('fangjianzhusu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>

                            <el-button v-if="isAuth('fangjianzhusu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                ...
                fangjianId : null,
                yonghuId : null,
                fangjianzhusuName : null,
                fangjianzhusuPhone : null,
                fangjianzhusuTime : null,
                zhusuNumber : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     '房间号': 'fangjianName',
                     '房间类型': 'fangjianTypes',
                     '房间价格(天)': 'fangjianMoney',
                     '员工姓名': 'yonghuName',
                     '头像': 'yonghuPhoto',
                     '联系方式': 'yonghuPhone',
                     '邮箱': 'yonghuEmail',
                //本表字段
                     '预约用户名称': "fangjianzhusuName",
                     '联系方式': "fangjianzhusuPhone",
                     '预约时间': "fangjianzhusuTime",
                     '住宿天数': "zhusuNumber",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'fangjianzhusu',//当前表表名,
                        sumColum : 'fangjianzhusu_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'fangjianzhusu_name',//分组字符串字段
                        // types : 'fangjianzhusu_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "fangjianzhusu_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['fangjianzhusuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "fangjianzhusu/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            fangjianzhusuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "fangjianzhusu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
