    ...
    */
    ...
    public R save(@RequestBody CaiwuEntity caiwu, HttpServletRequest request){
        ...
        Wrapper<CaiwuEntity> queryWrapper = new EntityWrapper<CaiwuEntity>()
            .eq("caiwu_name", caiwu.getCaiwuName())
            .eq("caiwu_types", caiwu.getCaiwuTypes())
            .eq("caiwu_text", caiwu.getCaiwuText())
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        CaiwuEntity caiwuEntity = caiwuService.selectOne(queryWrapper);
        if(caiwuEntity==null){
            caiwu.setInsertTime(new Date());
...
//            return R.error(511,"永远不会进入");
    ...
    */
    ...
    public R save( String fileName, HttpServletRequest request){
        ...
        try {
            List<CaiwuEntity> caiwuList = new ArrayList<>();//上传的东西
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            CaiwuEntity caiwuEntity = new CaiwuEntity();
//                            caiwuEntity.setCaiwuName(data.get(0));                    //标题 要改的
//                            caiwuEntity.setCaiwuTypes(Integer.valueOf(data.get(0)));   //财务类型 要改的
//                            caiwuEntity.setCaiwuMoney(data.get(0));                    //金额 要改的
//                            caiwuEntity.setInsertTime(date);//时间
//                            caiwuEntity.setCaiwuText(data.get(0));                    //详情 要改的
//                            caiwuEntity.setCreateTime(date);//时间
                            caiwuList.add(caiwuEntity);
                        ...
                        //查询是否重复
                        caiwuService.insertBatch(caiwuList);
                        return R.ok();
                    }
                }
            }
